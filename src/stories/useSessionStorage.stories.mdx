...
Hook for easily accessing `sessionStorage`, with a similar interface to `React.useState()`.
...
This will store and retrieve the value using the `name` key in `sessionStorage`.

> Note: If using TypeScript, you can provide a value type like so: `useSessionStorage<string>('name')`.

#### Providing A Default Value

You can also provide a default value for when the stored value is `undefined` (not currently set in `sessionStorage`):
...
```jsx
const [name, setName, initialized] = useSessionStorage('name', 'Guest', {
...
- When `shouldInitialize` is `true`, the prefix is used with the key to retrieve the stored value from `sessionStorage`.
- Then, `initialized` will return as `true`, and your value will be ready to use!

#### Clearing The Value

```jsx
const [name, setName, initialized, clear] = useSessionStorage('name', 'Guest');
```
...
```jsx
const [food, setFood, initialized, clear, prefixedKey] = useSessionStorage('food', 'Hamburger', {
  prefix: 'my-namespace',
});
```
...
| ---------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------- |
| `key`                  | `string`                             | **Required.** The key used to access the stored item in `sessionStorage`.                   |
...
| `shouldInitialize`             | `boolean`           | The value is not read from `sessionStorage`, or set, when this is `false`. Can be used to wait until `prefix` is ready (such as a user ID being loaded). Setter will have no effect when `false`. Default `true`. |
...
| ----- | ----------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0     | value       | `T` &#124; `null` &#124; `undefined`                 | The value of type `T` (the type of data you're storing) retrieved from `sessionStorage`, or the default (if provided to hook) when `undefined`.                                |
| 1     | setValue    | (value: `T` &#124; `null` &#124; `undefined`) ⇒ void | Sets the stored value to `sessionStorage`. This also notifies other hooks of the change via the emitter when `emitterDisabled` option is `false`.                              |
...
| 3     | clear       | `() => void`                                         | Delete the value from `sessionStorage`.                                                                                                                                        |
...
See the [Session Storage demo](https://justinmahar.github.io/react-storage-complete/?path=/story/stories-web-storage--session-storage) for a live interactive example of the hook in use.
